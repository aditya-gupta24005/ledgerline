...
# Needs: docker compose stack (incl. keycloak), settlement-service on 8082.
set -euo pipefail
cd "$(dirname "$0")/.."
...
OSCAR=$(auth oscar); RITA=$(auth rita)
...
sleep 3
echo "== Latest dead letters (oscar, OPS)"
curl -s "$SETTLEMENT_URL/api/v1/ops/dead-letters?limit=3" -H "$OSCAR"; echo
...
sleep 2
COORDINATES="$(curl -s "$SETTLEMENT_URL/api/v1/ops/dead-letters?limit=50" -H "$OSCAR" | python3 -c "
import json, sys
...
curl -s -X POST "$SETTLEMENT_URL/api/v1/ops/dead-letters/$PARTITION/$OFFSET/replay" -H "$OSCAR"; echo
sleep 3
echo "== replay-buyer balances as seen by rita (ACME +10, USD -1015)"
curl -s "$SETTLEMENT_URL/api/v1/accounts/replay-buyer/balances" -H "$RITA"; echo
